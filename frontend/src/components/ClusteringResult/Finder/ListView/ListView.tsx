...
const ListView = forwardRef<HTMLDivElement, listViewProps>(
  (
    ...
    // スクロール位置を復元
    useEffect(() => {
      if (localRef.current && scrollPosition > 0) {
        // 少し遅延させてDOMがレンダリングされた後にスクロール
        setTimeout(() => {
          if (localRef.current) {
    ...
    }, [currentFolder, scrollPosition]);

    // スクロールイベントハンドラー
    const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
      if (onScroll && localRef.current) {
        onScroll(localRef.current.scrollTop);
      }
    };
    ...
    return (
      <div className="list-view-main" ref={localRef} onScroll={handleScroll}>
        {folders.map((foldername, idx) => {
          ...
          return (
            <div
              key={idx}
              className={`${
                idx % 2 === 0 ? "list-view-item" : "list-view-item-odd"
              } ${isSelected ? "selected" : ""}`}
              onClick={() => {
                if (editingIndex === idx) {
                  // 編集モード時はクリックを無効化
                ...
                if (isLeaf) {
                  ...
                  // クリックされたフォルダがis_leafかどうかをチェック
                  const clickedFolderIsLeaf = isLeafFunction(
                    result,
                    foldername
                  );

                  // 計測モードの場合
                  if (isMeasuring && currentFolder) {
                    // フォルダクリックを記録
                    if (onFolderClick) {
                      onFolderClick(foldername, currentFolder, "list", false);
                    }
                    // リーフフォルダを展開した場合はカウント
                    if (clickedFolderIsLeaf && onLeafFolderOpen) {
                      onLeafFolderOpen(foldername);
                    }
                  }
